  ...
  return <div>
    ...
    {detailItem[index] ? <div className='px-52 mt-20'>
      <div className='card box-border w-60 p-4 border-4 mr-10' key={detailItem[index].id}>
        <Image className="rounded" src={detailItem[index].image} alt="imgAlt" width="300"
          height="400"  />
        <div className="mt-2  ml-16 card-detail ">
          <div>
            <div className="title text-slate-700 uppercase font-black tracking-wider">{detailItem[index].title}</div>
            <div className="card-price mt-16 float-right text-slate-600">{detailItem[index].price}TL</div>
          </div>
        </div>
      </div>
    </div> : <div></div>}
